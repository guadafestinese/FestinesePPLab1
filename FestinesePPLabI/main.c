...
int main()
{
    ...
    eNotebook notebooks[TAMN];/* ={//ID MODELO MARC TIP PRECIO
    								{1,	"Uno",	2,	3,	2000,	0},
									{2, "Dos",	1,	3,	3000,	0},
									{3,	"Tres", 1,	3,	3500,	0},
									{4,	"Cuatro",2,	4,	5000,	0},
    ...
    do{
		switch(menu()){
			case 1:
				if(altaNotebook(proximoIdNotebook, notebooks, TAMN, tipos, TAMT, marcas, TAMM) == 1){
					proximoIdMarca++;
					proximoIdTipo++;
					proximoIdNotebook++;
				}
			...
			case 7:
				...
				break;
			case 8:
			   if(altaTrabajo(proximoIdTrabajo, notebooks, TAMN, tipos, TAMT, marcas, TAMM, servicios, TAMS, trabajos, TAMTR)){
				   proximoIdServicio++;
				   proximoIdTrabajo++;
			   }
			   break;
